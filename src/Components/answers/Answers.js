    ...
    <div className='answers'>
      ...
      <div className='answers__body'>
        {answers.map((item, index) => {
          ...
          return (
            <div
              className='answers__body__item'
              key={`${index}${correct_answer}`}
            >
                ...
                return (
                  <div className='each__item' key={item}>
                    <div className='answer'>
                      <p>{`${index + 1}. ${item}`}</p>
                      {ans.indexOf(your_answer) === index && (
                        <div className='indicator'>
                          <p className='common'>Your Answer</p>
                          {show ? (
                            <FaCheck className='blue' />
                          ) : (
                            <CgClose className='red' />
                          )}
                        </div>
                      )}
                      <p className='green common'>
                        {correct_answer === item && !show && 'Correct Answer'}
                      </p>
                    </div>
                  </div>
                );
